...
(float exectime) launch_wrapper(string run_id, int params[], boolean input = false, int count = 0)
{
	...
	if (count < time_limit)
	{
		...
		else
		{
			...
			// Commands
			string cmds[];
			cmds[0] = "../../../../../gray-scott/build/gray-scott";
			cmds[1] = "../../../../../gray-scott/build/pdf_calc";
			...
			printf("swift: multiple launching: %s (%s, %s), %s (%s, %s)", cmds[0], envs[0][4], envs[0][5], cmds[1], envs[1][4], envs[1][5]);
			sleep(1) =>
...
(void v) failure(string run_id, int params[])
{
	string turbine_output = getenv("TURBINE_OUTPUT");
	string dir = "%s/run/%s" % (turbine_output, run_id);
	string output = "%0.3i\t%0.3i\t%0.4i\t%0.2i\t%0.4i\t%0.2i\t%s"
	...
	if (count < time_limit)
	{
		...
		else
		{
			...
			if (exectime >= 0.0) {
				...
				string output = "%0.3i\t%0.3i\t%0.4i\t%0.2i\t%0.4i\t%0.2i\t%f"
					% (params[0], params[1], params[2], params[3], params[4], params[5], exectime);
				file out <dir/"time.txt"> = write(output);
			}
			else
...
main()
{
	...
	foreach i in [0 : sample_num - 1 : 1]
	{
		...
		if ((params[3] <= ppw) && (params[5] <= ppw))
		{
			...
			if (nwork <= workers)
			{
				exectime[i] = launch_wrapper("%0.3i_%0.3i_%0.4i_%0.2i_%0.4i_%0.2i" 
						% (params[0], params[1], params[2], params[3], params[4], params[5]), 
						params, false);
